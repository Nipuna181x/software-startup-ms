    ...
    <div class="w-full overflow-clip bg-white">
        ...
        <header x-data="{ open: false }" class="relative z-30 mx-auto max-w-7xl border-b border-zinc-100 px-5 sm:px-10">
            <div class="flex h-22 items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2 sm:gap-4">
                    @auth
                        <a href="{{ route('dashboard') }}" wire:navigate class="button-dark">Open workspace <flux:icon icon="arrow-up-right" class="size-4 shrink-0" aria-hidden="true" /></a>
                    @else
                        <a href="{{ route('login') }}" wire:navigate class="hidden text-sm font-medium sm:block">Log in</a>
                        <a href="{{ route('register') }}" wire:navigate class="button-outline !px-4">Get started <flux:icon icon="arrow-up-right" class="size-4 shrink-0" aria-hidden="true" /></a>
                    @endauth
        ...
        <footer class="mx-auto max-w-7xl px-5 pt-20 pb-7 sm:px-10">
            <div class="grid gap-10 border-b border-zinc-200 pb-12 md:grid-cols-[1.8fr_1fr_1fr]">
                ...
                <div><h2 class="mb-5 text-sm font-semibold">Make a fresh start</h2><div class="grid gap-3 text-sm text-zinc-600"><a href="{{ route('register') }}" wire:navigate>Create a workspace <flux:icon icon="arrow-up-right" class="inline-block size-4 shrink-0 align-text-bottom" aria-hidden="true" /></a><a href="{{ route('login') }}" wire:navigate>Log in to your account</a></div></div>
            </div>
